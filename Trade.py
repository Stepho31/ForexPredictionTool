...
def fetch_forex_data(pair, time_frame):
    ...
    start_date = end_date - timedelta(days=7)  # Adjust for recent data
    start_date_str = start_date.strftime("%Y-%m-%d")
    end_date_str = end_date.strftime("%Y-%m-%d")

    url = f"https://api.polygon.io/v2/aggs/ticker/{ticker}/range/{time_frame}/minute/{start_date_str}/{end_date_str}?apiKey={api_key}"
